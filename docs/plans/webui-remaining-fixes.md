# Plan: WebUI 剩余问题修复（4 项）

## Summary

修复 Web 前端 4 个问题：Dockview Tab 样式（CSS 覆盖）、CWD 获取失败（后端 RPC 对齐活跃会话）、终端面板占位（重写 TerminalPanel 接入 xterm.js）、消息渲染折叠逻辑（三级折叠 + 工具合并 + 迭代分组）。
...
| 3 | 终端功能异常 | TerminalPanel.tsx 仍是占位 stub，从不接入 xterm/TerminalWS | terminal-audit |
| 4 | 消息渲染/折叠逻辑 | minimal 等同 all；工具逐个独立成卡无合并；流式中无迭代分组；无总耗时数据 | render-logic-audit |
...
- What: 去掉 createTerminal 中硬编码的 `channel:'web'`，改为无参或只传 chat_id
- Why: 让后端按活跃会话解析，而非被硬编码的 channel 绕过

#### `web/src/components/sidebar/SessionInfo.tsx`
- What: set_cwd 调用去掉 `channel:'web'`
- Why: 同上

### 3. 终端面板（重写 TerminalPanel）

#### `web/src/workspace/panels/TerminalPanel.tsx`
- What: 从占位 stub 重写为真实 xterm.js 面板：
  1. 从 `params.terminalId` 取前端终端 id
  2. `terminalStore.getSession(id)` 拿 `tid`
  3. `useEffect` 中 `new Terminal({...})` + `FitAddon` + `WebLinksAddon` 挂到容器 ref
  4. `new TerminalWS(tid, {...})` 建立 WS 连接
  5. `term.onData → terminalWS.sendStdin`
  6. `term.onResize → terminalWS.resize`
  7. `onOpen → updateStatus('connected') + fit`
  8. `onExit/onError → updateStatus('exited'/'error')`
  9. `ResizeObserver → fitAddon.fit()`
  10. cleanup: `terminalWS.dispose()` + `term.dispose()` + `terminalStore.deleteBackend(tid)` + `terminalStore.remove(id)`
- Why: TerminalPanel 从未接入 xterm/TerminalWS，是终端功能异常的直接原因
...
- **TerminalPanel 重写**：xterm 在 dockview 隔离 React root 中渲染，需确保 context 桥接正确（已在之前修复中添加）。
